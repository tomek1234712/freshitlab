...
$Post_Type_Archive_Links = new Post_Type_Archive_Links();
...
class Post_Type_Archive_Links {
	...
	public function init() {
		if ( ! $this->ininited ) {
			$this->ininited = true;
			$this->enable( dirname( INC_PLUGIN_DIR . DS . "post-type-archive-links" ) );
	...
	public function metabox_script( $hook ) {
		...
		wp_register_script(
			'hptal-ajax-script',
            get_template_directory_uri() . DS . 'plugins' . DS . 'post-type-archive-links' . DS . "metabox{$suffix}.js",
			array( 'jquery' ),
			filemtime( INC_PLUGIN_DIR . DS . "post-type-archive-links" . DS . "metabox{$suffix}.js" ),
			true
		);
